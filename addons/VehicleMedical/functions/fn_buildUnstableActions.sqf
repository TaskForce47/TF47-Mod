 ...
 *
 * Example:
 * [_player,_target,[_patient]] call TF47_fnc_buildUnstableActions
 *
 * Public: Yes
 */
...
if(GVAR(Unstable_TrackDead) && !alive _patient) then {
    ...
    private _action = ["TF47_Dead", [LSTRING(Unstable,Dead)] call FUNC(cachedLocalisationCall), QUOTE(ICON_PATH(dead)), {
            params ["_target", "_player", "_parameters"];
...
if(_canTakeDogtag && {(GVAR(Unstable_DogtagsDeadOnly) && !alive _patient) || !GVAR(Unstable_DogtagsDeadOnly)}) then {
    ...
    private _action = ["TF47_Dogtags_Take", _taken, QUOTE(ICON_PATH(dogtags)), {
            params ["_target", "_player", "_parameters"];
            _parameters params ["_patient"];
...
if (_cardiacArrest) then {
    ...
    _action = ["TF47_Cardiac", [LSTRING(Unstable,Cardiac_Arrest)] call FUNC(cachedLocalisationCall), QUOTE(ICON_PATH(cardiac_arrest_red)), {
            params ["_target", "_player", "_parameters"];
            _parameters params ["_patient"];
            [_patient] call FUNC(openMedicalMenu);
        }, {true}, {}, [_patient]] call ace_interact_menu_fnc_createAction;
...
if(GVAR(EnableSupportKAT)) then {
    ...
    if(GVAR(Unstable_TrackSpO2) && _spO2 < 85) then {
        _action = ["TF47_KAT_SpO2", format[[LSTRING(Unstable_KAT,SpO2)] call FUNC(cachedLocalisationCall), round _spO2, "%"], QUOTE(ICON_PATH(kat_spO2_low)), {
            params ["_target", "_player", "_parameters"];
            _parameters params ["_patient"];
            [_patient] call FUNC(openMedicalMenu);
        }, {true}, {}, [_patient]] call ace_interact_menu_fnc_createAction;
    ...
    if(GVAR(Unstable_TrackAllPneumothorax) && (_pneumothorax || _hemopneumothorax || _tensionPneumothorax)) then {
        ...
        _action = ["TF47_KAT_Pneumothorax", _name, QUOTE(ICON_PATH(kat_pneumothorax)), {
            params ["_target", "_player", "_parameters"];
            _parameters params ["_patient"];
            [_patient] call FUNC(openMedicalMenu);
        }, {true}, {}, [_patient]] call ace_interact_menu_fnc_createAction;
    ...
    if(GVAR(Unstable_TrackAirwayBlocked) && (_airwayObstruction || _airWayOcclusion)) then {
        ...
        _action = ["TF47_KAT_AirwayBlocked", _name, QUOTE(ICON_PATH(kat_airway_blocked)), {
            params ["_target", "_player", "_parameters"];
            _parameters params ["_patient"];
            [_patient] call FUNC(openMedicalMenu);
        }, {true}, {}, [_patient]] call ace_interact_menu_fnc_createAction;
...
if (_isBleeding) then {
    ...
    _action = ["TF47_Bleeding", [LSTRING(Unstable,Bleeding)] call FUNC(cachedLocalisationCall), _icon, {
            params ["_target", "_player", "_parameters"];
            _parameters params ["_patient"];
            [_patient] call FUNC(openMedicalMenu);
        }, {true}, {}, [_patient]] call ace_interact_menu_fnc_createAction;
...
if(_hasLowHR) then {
    ...
    private _action = ["TF47_LowHR", format[[LSTRING(Unstable,Low_Heart_Rate)] call FUNC(cachedLocalisationCall), _hr], QUOTE(ICON_PATH(hr_low)), {
            params ["_target", "_player", "_parameters"];
            _parameters params ["_patient"];
            [_patient] call FUNC(openMedicalMenu);
        }, {true}, {}, [_patient]] call ace_interact_menu_fnc_createAction;
...
if(_hasLowBP) then {
    ...
    private _action = ["TF47_LowBP", _name, QUOTE(ICON_PATH(bp_low)), {
            params ["_target", "_player", "_parameters"];
            _parameters params ["_patient"];
            [_patient] call FUNC(openMedicalMenu);
        }, {true}, {}, [_patient]] call ace_interact_menu_fnc_createAction;
...
if (_isUncon) then {
    ...
    private _action = ["TF47_Sleepy", [LSTRING(Unstable,Unconscious)] call FUNC(cachedLocalisationCall), QUOTE(ICON_PATH(unconscious_white)), {
            params ["_target", "_player", "_parameters"];
            _parameters params ["_patient"];
            [_patient] call FUNC(openMedicalMenu);
        }, {true}, {}, [_patient]] call ace_interact_menu_fnc_createAction;
...
if(GVAR(Unstable_TrackLegFractures) && [_patient] call FUNC(hasLegFractures)) then {
    ...
    private _action = ["TF47_Fractures", _fracturesMessage, QUOTE(ICON_PATH(fracture)), {
            params ["_target", "_player", "_parameters"];
            _parameters params ["_patient"];
            [_patient] call FUNC(openMedicalMenu);
        }, {true}, {}, [_patient]] call ace_interact_menu_fnc_createAction;
...
if(GVAR(Unstable_TrackLegSplints) && [_patient, true] call FUNC(hasLegFractures)) then {
    ...
    private _action = ["TF47_Splinted_Fractures", _fracturesMessage, QUOTE(ICON_PATH(splint)), {
            params ["_target", "_player", "_parameters"];
            _parameters params ["_patient"];
            [_patient] call FUNC(openMedicalMenu);
        }, {true}, {}, [_patient]] call ace_interact_menu_fnc_createAction;
