 ...
 *
 * Example:
 * [_player,_target,[_patient]] call TF47_fnc_buildStableActions
 *
 * Public: Yes
 */
...
if(_needsBandage) then {
    ...
    private _action = ["TF47_Bandage", format[[LSTRING(Stable,Bandage)] call FUNC(cachedLocalisationCall), _requiredBandages] , QUOTE(ICON_PATH(bandage)), {
            params ["_target", "_player", "_parameters"];
...
if (_needsStitch) then {
    ...
    private _action = ["TF47_Stitch", format[[LSTRING(Stable,Stitch)] call FUNC(cachedLocalisationCall), count _stitchWounds] , QUOTE(ICON_PATH(stitch)), {
            params ["_target", "_player", "_parameters"];
            _parameters params ["_patient"];
            [_patient] call FUNC(openMedicalMenu);
        }, {true}, {}, [_patient]] call ace_interact_menu_fnc_createAction;
...
if(_hasLowHR) then {
    ...
    private _action = ["TF47_LowHR", format[[LSTRING(Stable,Low_Heart_Rate)] call FUNC(cachedLocalisationCall), _hr], QUOTE(ICON_PATH(hr_low)), {
            params ["_target", "_player", "_parameters"];
            _parameters params ["_patient"];
            [_patient] call FUNC(openMedicalMenu);
        }, {true}, {}, [_patient]] call ace_interact_menu_fnc_createAction;
...
if(_tourniquets) then {
    ...
    private _action = ["TF47_Tourniquets", format[[LSTRING(Stable,Tourniquets)] call FUNC(cachedLocalisationCall), _amount], QUOTE(ICON_PATH(tourniquet)), {
            params ["_target", "_player", "_parameters"];
            _parameters params ["_patient"];
            [_patient] call FUNC(openMedicalMenu);
        }, {true}, {}, [_patient]] call ace_interact_menu_fnc_createAction;
