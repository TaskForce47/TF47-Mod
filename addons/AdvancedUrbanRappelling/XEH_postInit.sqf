...
[
        ...
        {
                if ([player] call TF47_fnc_AUR_Rappel_Attach_Action_Check) exitWith {[player] spawn TF47_fnc_AUR_Rappel_Attach_Action};        // try to attach to already existing rope first
                if ([player] call TF47_fnc_AUR_Rappel_Action_Check) exitWith {[player] spawn TF47_fnc_AUR_Rappel_Action};                      // rappel, if possible
                if ([player] call TF47_fnc_AUR_Rappel_Detach_Action_Check) then {[player] spawn TF47_fnc_AUR_Rappel_Detach_Action};            // detach from rope, if rappelling
...
[
        ...
        {
                if ([player] call TF47_fnc_AUR_Detach_Rope_Action_Check) then {[player] spawn TF47_fnc_AUR_Detach_Rope_Action};
